...
mlp = joblib.load('modelNumbers.joblib') # Carga del modelo. 
skl = joblib.load('ScalerNumbers.joblib') # Carga del modelo.
...
for iP in imgPath:
    ...
    img = cv2.imread(iP,0)
    img = cv2.resize(img, (27,58))
    imgColor = cv2.resize(imgColor, (27,58))
    ...
    if len(contours) > 0:
        for cnt in contours:
            ...
            if(areaHW>1000):
                ...
                cv2.imshow("imgColor",imgRoiResize)
