...
class textBoxLabel(wx.Dialog):
    def __init__(self, parent, id, title):
        ...
        wx.Dialog.__init__(self, parent, id, title, size = (285, 120))
        self.Center()

        self.listClass = ["a", "b", "c", "d","p", "r", "e", "f"]
        self.rbClass = wx.RadioBox(self, pos=(10, 0), choices = self.listClass, majorDimension=8, style = wx.RA_SPECIFY_COLS)
...
def click_mouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONUP and not bandClass:
        ...
        if labelOK and (label == 'a' or label == 'b' or label == 'c' or label == 'd' or label == 'p' or label == 'r' or label == 'e' or label == 'f'):
            labelOK = False
            tagged = True
            xmax = x
            ymax = y
